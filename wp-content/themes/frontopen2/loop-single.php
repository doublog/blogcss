...
<?php if ( have_posts() ) while ( have_posts() ) : the_post(); ?>
                ...
                <div class="datetime"><?php the_time('Y') ?><br /><?php the_time('m-d') ?></div>
					...
					<?php }?>
                        ...
                        <p>&gt;&gt; 转载请注明: <?php if(get_option('themes_fo2_zhuanzai')){echo get_option('themes_fo2_zhuanzai');}else{ ?><?php the_author_posts_link(); ?> <?php the_time('Y年m月d日') ?> 于 <a href="<?php echo home_url(); ?>/" title="访问<?php bloginfo('name');?>"><?php bloginfo('name');?></a> 发表<?php }?></p>
                        </div>
