...
<?php $t1=$post->post_date;
	  ...
	  if(get_option('themes_fo2_view_time') && getPostViews(get_the_ID())){echo "<span class='title_hot'>" . getPostViews(get_the_ID()) . " 人踩过</span>";}else{if(get_option('themes_fo2_view_num')){if(getPostViews(get_the_ID()) > get_option('themes_fo2_view_num')){echo " <span class='title_hot'>HOT</span>";};}else{if(getPostViews(get_the_ID()) > 10){echo " <span class='title_hot'>人踩过</span>";}}}
	   ?>
...
<div class="post_box" <?php if($ifpic == 'checked'){ echo "style='float:right'" ;}?>>
    ...
    <a href="<?php the_permalink(); ?>" class="disp_a" rel="bookmark" title="<?php printf( esc_attr__( 'Permalink to %s', 'frontopen' ), the_title_attribute( 'echo=0' ) ); ?>"><?php if($ifpic != 'checked'){ ?>
                ...
                <?php the_post_thumbnail('thumbnail');?><?php echo dm_strimwidth(strip_tags($post->post_content),0,$dis_num,'....');?>
                <?php } elseif(catch_that_image()){?>
                <?php ?><img src="<?php echo catch_that_image() ?>" /><?php echo dm_strimwidth(strip_tags($post->post_content),0,$dis_num,'....'); ?>
                <?php } else {echo dm_strimwidth(strip_tags($post->post_content),0,$dis_num+50,'....');} ?>
     <?php }?>                          
