...
function add_checkbox() {   
   if (!is_user_logged_in()) {
      $content.= '<input type="checkbox" name="access_submit" id="access_submit" value="access_submit" style="width: 25px;" /><label for="access_submit" style="margin-left:15px;"> 同意并回复</label>';   
...
$buffer_out = preg_replace('/https:\/\/www.doublog\.com\/wp-([^"\']*?)\.(jpg|png|gif|css|woff|woff2|ttf|svg|eot)/i','https://cdn.doublog.com/wp-$1.$2',$buffer);
return $buffer_out;
}
#域名301
function url301(){
if(strtolower($_SERVER['SERVER_NAME'])=='cdn.doublog.com')
{
...
header('Location:https://www.doublog.com'.$URIRedirect);
exit();
}
}
...
function the_content_Jump($content)
{
	...
	if($matches)
	{
		...
		{
			if(strpos($val,'://')!==false && strpos($val,'nofollow')===false && strpos($val,home_url())===false && strpos($val,'https://www.doublog.com')===false )
			{
