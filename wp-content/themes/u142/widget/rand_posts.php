...
class u142_rand2_post_list extends WP_Widget {

	function u142_rand2_post_list(){

		$widget_ops = array('description' => '非首页随机文章列表');

		parent::WP_Widget('u142_rand2_post_list',$name='u142-非首页随机文章',$widget_ops);
	...
	function form($instance){
		...
		$instance = wp_parse_args((array)$instance,array('title'=>'随机文章','showPosts'=>10));//默认值
	...
	function the_rand2_posts($args = ''){
		...
		$rand2_post_query = new WP_Query($args_post);

		if($rand2_post_query->have_posts()){

			$i=0;

			echo '<div id="tb-post" class="clearfix"><ul class="hot_list">';

			while($rand2_post_query->have_posts()){

				$i++;

				$rand2_post_query->the_post();
				...
				$featured_image_url = wp_get_attachment_image_src( get_post_thumbnail_id($post->ID), 'featured-thumbnail'); 
				...
				$rand2_post_content = mb_strimwidth(strip_tags(apply_filters('the_content', get_the_content())), 0, 46,"…");
				...
				if($i>1){
					...
					echo '<li class="items"><div class="pbox"><a href="'.get_permalink().'" title="'.get_the_title().'">'.$images.'</a></div><div class="tinfo"><a href="'.get_permalink().'">'.get_the_title().'</a><p>'.$rand2_post_content.'</p></div></li>';				
				}
			}		
	...
	function widget($args, $instance){
		...
		self::the_rand2_posts("showPosts=$showPosts");
...
//激活小工具
register_widget('u142_rand2_post_list');
?>