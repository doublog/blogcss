...
class u142_index_latest_post_list extends WP_Widget {

	function u142_index_latest_post_list(){

		$widget_ops = array('description' => '首页最新文章列表');

		parent::WP_Widget('u142_index_latest_post_list',$name='u142-首页最新文章',$widget_ops);
	...
	function form($instance){
		...
		$instance = wp_parse_args((array)$instance,array('title'=>'最新文章','showPosts'=>6));//默认值
	...
	function the_index_latest_posts($args = ''){
		...
		$index_latest_post_query = new WP_Query($args_post);

		if($index_latest_post_query->have_posts()){		

			echo '<div id="ta-post" class="clearfix"><ul>';

			while($index_latest_post_query->have_posts()){				

				$index_latest_post_query->the_post();
	...
	function widget($args, $instance){
		...
		self::the_index_latest_posts("showPosts=$showPosts");
...
//激活小工具
register_widget('u142_index_latest_post_list');
?>