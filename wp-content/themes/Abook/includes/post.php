...
function WinyskyFeedAdditional($content){
	if(is_single() or is_feed()){//文章开头广告
		...
		if(is_single()&&(get_option('Abook_postbottom') == 'TRUE')){
...
function post_thumbnail( $width = "220px",$height = "100px" ){
    global $post;
        ...
        ob_end_clean();
		//匹配 <img>
	preg_match_all('~<img [^\>]*>~',$post->post_content,$index_matches);//获取日志中第一张图片
	$first_image = $index_matches[0][0];
	//匹配 src
	preg_match_all('~src=[\'"]([^\'"]+)[\'"]~', $first_image, $index_matches);
	$first_img_src=$index_matches[1][0];//获取该图片 src

        if(empty($first_img_src) ){//如果日志中没有图片，则随机显示

           $first_img_src = get_bloginfo('template_directory').'/images/default_thumb/'.rand(1,22).'.jpg';
		 
        }
		  $post_timthumb = '<a title="'.$post->post_title.'" href="'.get_permalink().'"><img src="'.get_bloginfo("template_url").'/timthumb.php?src='.$first_img_src.'&amp;h='.$height.'&amp;w='.$width.'&amp;zc=1" alt="" class="thumb" /></a>';
